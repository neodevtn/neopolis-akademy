...
const videoBlock: BlockTypeDefinition = {
  ...
  schema: [
    ...
    { key: "sourceUrl", label: { en: "Source URL", fr: "URL de la source" }, type: "i18n_text", required: true, placeholder: "https://…", helpText: { en: "YouTube URL or ID for the selected language. For hosted media, use the same managed URL in both language tabs. Legacy playback fields are updated automatically.", fr: "URL ou identifiant YouTube pour la langue sélectionnée. Pour un média hébergé, utilisez la même URL gérée dans les deux onglets. Les champs techniques historiques sont mis à jour automatiquement." } },
...
const cloudExerciseBlock: BlockTypeDefinition = {
  ...
  schema: [
    ...
    { key: "postRevealReflectionTitle", label: { en: "Post-solution reflection title", fr: "Titre de l’auto-évaluation après correction" }, type: "i18n_text" },
    { key: "postRevealReflectionOptions", label: { en: "Post-solution reflection options", fr: "Choix d’auto-évaluation après correction" }, type: "array", helpText: { en: "Optional learner reflection choices revealed after the solution. Selecting a choice can be required before completion.", fr: "Choix de réflexion facultatifs révélés après la correction. Leur sélection peut être requise avant la complétion." }, arrayItemSchema: [
      { key: "id", label: { en: "Stable option ID", fr: "Identifiant stable du choix" }, type: "text", required: true },
      { key: "label", label: { en: "Learner-facing label", fr: "Libellé apprenant" }, type: "i18n_text", required: true },
      { key: "feedback", label: { en: "Reflection feedback", fr: "Retour d’auto-évaluation" }, type: "i18n_textarea" },
      { key: "passes", label: { en: "Completes the activity", fr: "Termine l’activité" }, type: "boolean", defaultValue: false, helpText: { en: "Only a choice marked complete unlocks the next chapter; other choices invite the learner to revise and resubmit.", fr: "Seul un choix marqué comme terminé déverrouille le chapitre suivant ; les autres invitent l’apprenant à réviser puis soumettre à nouveau." } },
    ]},
    { key: "requirePostRevealReflection", label: { en: "Require a reflection choice", fr: "Exiger un choix d’auto-évaluation" }, type: "boolean", defaultValue: false, helpText: { en: "Keeps the progression locked until the learner selects one reflection result after viewing the solution.", fr: "Maintient la progression verrouillée jusqu’à la sélection d’un résultat d’auto-évaluation après lecture de la correction." } },
...
const genericLearningBlocks: BlockTypeDefinition[] = [
  ...
  },
  {
    ...
    schema: [{ key: "id", label: { en: "Stable action ID", fr: "Identifiant stable" }, type: "text", required: true }, { key: "title", label: { en: "Screen title", fr: "Titre d’écran" }, type: "i18n_text", required: true }, { key: "steps", label: { en: "Steps", fr: "Étapes" }, type: "json", required: true }, { key: "expectedEvidence", label: { en: "Expected evidence", fr: "Preuve attendue" }, type: "i18n_textarea" }, { key: "resources", label: { en: "Required downloadable files", fr: "Fichiers téléchargeables requis" }, type: "json", helpText: { en: "Every learner file named in an action must use a managed media-library URL.", fr: "Chaque fichier cité à l’apprenant doit utiliser une URL de la médiathèque gérée." } }], defaultData: { type: "guided_action", id: "", title: { en: "Guided action", fr: "Action guidée" }, steps: [] },
  },
  {
