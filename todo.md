...
- [ ] Exécuter un balayage Anthropic accéléré sur les cours restants : inventorier par analyse statique les troncatures, activités racine non standard, durées incohérentes, reliquats français manifestes, pseudo-tableaux et repli média sans provenance ; appliquer seulement les correctifs réexécutables déterministes puis publier une validation transversale unique
- [ ] Appliquer la règle 80/20 au balayage Anthropic : traiter d’abord les défauts visibles ou bloquants récurrents et reporter les micro-ajustements non fonctionnels après la fenêtre de stabilisation
- [ ] Normaliser de manière déterministe les reliquats génériques anglais répétés dans les variantes françaises des cours Anthropic non clôturés (`system prompt`, `workflow`, `Watch Out`, `What good looks like`, `few-shot examples`, `output constraints`) sans modifier les noms produits Anthropic ni les extraits de code
