...
- [ ] Corriger Developer Foundations cours 2 « Production-Grade Prompting, Agents & Tool Use » : durée officielle 209 min, 12 écrans, TP locaux exécutables et checkpoints agent loop/tool use, en blocs standards et avec verrouillage séquentiel ; regrouper les anomalies démontrées avant validation et contrôle public uniques
- [ ] Corriger le lot critique Developer 2 : durée officielle et introduction, huit exercices racine `free_text` non standards/mal rattachés, contradictions Extended Thinking/streaming, pseudo-tableaux, titres concaténés, troncatures de cartes, reliquats anglais et huit vidéos sans provenance
- [ ] Convertir les pseudo-tableaux restants de l’écran Prompting Craft Developer 2 en sections Markdown lisibles, y compris les en-têtes concaténés `System Prompts / XML Tags / Few-shot Examples / Output Constraints`, et localiser les occurrences génériques restantes de `output constraint`, `system prompt` et `Few-shot examples`
...
- [ ] Corriger le reliquat générique « system prompt » de l’option de checkpoint Prompting Craft Developer 2, puis rejouer le rendu public ciblé
