...
- [ ] Monter TekTek avec la leçon actuellement affichée lorsqu’une URL de cours n’a pas de paramètre `lesson`, afin que l’action du centre d’assistance ouvre réellement le coach dans une session apprenant publiée — prévisualisation validée ; contrôle public restant requis
...
- [ ] Tester TekTek sur le domaine public avec un compte apprenant depuis l’URL canonique du cours sans paramètre `lesson`, confirmer l’ouverture via le centre d’assistance et archiver la preuve dans `docs/support-feedback-sentry.md`
- [ ] Après propagation publique, vérifier qu’une question TekTek depuis cette URL sans `lesson` répond avec le bon contexte de leçon affiché, sans erreur console
