...
- [ ] Étendre la normalisation 80/20 aux reliquats français génériques fréquents (`best practices`, `key takeaways`, `human-in-the-loop`, `guardrail`, `rollout plan`, `model card`) sans traduire les noms produits Anthropic ni les identifiants techniques
- [ ] Remplacer en priorité les exercices racine `free_text` invisibles ou non obligatoires des cours Anthropic restants par un checkpoint standard à choix unique, généré par Claude Sonnet à partir des notions du cours et rattaché au premier écran pédagogique pertinent
