...
Les champs français restés intégralement en anglais dans `Claude 101`, `Claude Code 101` et `Claude Code in Action` sont traduits par lots avec un modèle de traduction, une réponse JSON typée, des contrôles de longueur/confiance et une application atomique. Les commandes, chemins, code et libellés techniques nécessaires restent dans la langue source. L’inventaire est passé de 532 champs en double langue à 12 reliquats : chacun est désormais un exemple de code, une commande CLI ou une syntaxe de protocole qui doit rester littérale. Le test de non-régression impose zéro texte pédagogique identique dans les trois cours traduits.
