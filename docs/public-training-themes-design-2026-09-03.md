...
Après propagation du checkpoint `8c650dda`, la sonde a été rejouée sur `https://akademy.neodev.click`. Les six contrôles de l’index et de la page Finance, répartis sur 1280 × 720, 390 × 844 et 375 × 667, sont tous passés : HTTP 200, titre attendu, contenu présent, canonical, Open Graph, JSON-LD et absence de débordement (`scrollWidth = clientWidth`). La route inconnue renvoie HTTP 404 avec `noindex`; le sitemap publié référence l’index et le thème Finance. Les deux pages ont également été consultées visuellement dans le navigateur de production, avec le logo officiel Neopolis et les liens attendus.
