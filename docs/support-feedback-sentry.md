...
### Contrôle public par rôle après publication

Après publication du checkpoint `ae0a8e22` et rafraîchissement explicite du bundle public, un compte apprenant de démonstration a ouvert l’URL canonique du cours 1 sans paramètre de leçon. Le centre d’assistance a ouvert TekTek, dont la réponse au Framework 4D cite les sources du cours et couvre Delegation, Description, Discernment et Diligence. Un signalement de contrôle explicitement marqué à ignorer a reçu la confirmation d’envoi sans erreur console. L’API Sentry on-premise confirme ensuite ce feedback public et la résolution HTTP `200` de son événement associé.

Une session administrateur séparée a confirmé les quatre canaux du même centre. TekTek y répond également au Framework 4D avec des sources ouvrables ; l’ouverture d’une source a effectivement navigué vers la leçon pédagogique référencée, dont le contenu Diligence s’est chargé en mode revue. Le formulaire de signalement s’ouvre puis se ferme sans erreur bloquante. Aucun deuxième feedback administrateur n’a été créé afin de ne pas encombrer l’inbox. Les contrôles ont porté sur `https://akademy.neodev.click/training/claude_certified_architect_foundations/claude_certified_architect_foundations__01` ; aucune donnée personnelle, contenu de test ou identifiant Sentry n’est archivé ici.

> Les deux premiers essais publics ont précédé la propagation complète du bundle. Seul le signalement post-propagation, confirmé à la fois par l’interface et par l’API de feedback avec événement résolvable, constitue la preuve de disponibilité publiée.
