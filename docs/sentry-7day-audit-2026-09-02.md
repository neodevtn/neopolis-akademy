...
Après publication du checkpoint `95ee7a56`, les contrôles ont été rejoués sur `https://akademy.neodev.click`. L’orientation confirme publiquement cinq sélections, l’inactivation du sixième choix, le message de capacité et l’absence d’erreur de page, sans enregistrer le formulaire. Le premier module du cours Projector concerné rend deux activités avec audio et slides, déclenche la lecture et ne produit aucune exception média non gérée. La récupération de bundles est validée par ses tests déterministes couvrant chaque signature et par le chargement public sain des parcours testés ; elle ne peut être déclenchée artificiellement sans servir à nouveau un bundle périmé.

Le rechargement de bundle ne peut volontairement pas être provoqué sur le domaine public : une telle démonstration imposerait de servir un fichier JavaScript périmé ou d’injecter un point de débogage qui déclencherait un rafraîchissement de navigateur. Ces deux options dégraderaient la sécurité ou l’expérience des apprenants. La preuve automatisée exécute donc la fonction réelle avec une fenêtre navigateur simulée : elle confirme l’ajout de `client-recovery` en préservant les paramètres de parcours, un seul rechargement pour une même signature, puis un second rechargement autorisé seulement pour une signature différente. Les 12 tests de ce module sont réussis ; le chargement nominal public est sain.

Une seconde preuve, `scripts/check-chunk-recovery-browser.mjs`, exécute le mécanisme réel dans un navigateur Chromium isolé sur la prévisualisation de développement. Elle confirme le premier rechargement `lazy-default`, le refus d’un doublon de même signature, puis le rechargement distinct `react-tree`, avec conservation des paramètres et aucune erreur de page. Son rapport non identifiant est archivé dans `docs/chunk-recovery-browser-qa.json`. Aucun mécanisme de test, route ou import n’a été publié dans l’application.
