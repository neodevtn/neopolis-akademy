...
Après confirmation explicite du propriétaire, une nouvelle vérification a confirmé zéro occurrence postérieure au déploiement du 7 septembre à 00:49 UTC. La première tentative de résolution d’un groupe a été refusée par Sentry avec HTTP 403 avant toute modification de statut ; l’opération doit reprendre uniquement avec la protection CSRF de la session authentifiée.

La protection CSRF a ensuite été appliquée à partir du cookie de session sans en exposer la valeur. Le groupe `removeChild` historique a été marqué résolu avec HTTP 200 ; sa fréquence et sa dernière occurrence sont restées inchangées.

Les dix autres groupes historiques ont ensuite été marqués résolus avec succès. La vérification finale de l’API Sentry retourne zéro issue non résolue sur les sept derniers jours et confirme les onze groupes au statut `resolved`. La dernière occurrence demeure celle du 6 septembre à 21:11 UTC, donc antérieure au déploiement corrigé ; aucune nouvelle erreur n’est apparue depuis.
