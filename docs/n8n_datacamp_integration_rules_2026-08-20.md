...
Les formulations encore dépendantes de la VM DataCamp sont adaptées au rendu : l’apprenant n’est plus présenté comme connecté automatiquement ni renvoyé vers le bureau de la VM. Le bloc TP demande maintenant de se connecter à n8n Cloud ou Docker, puis de reconstruire explicitement le workflow référencé à partir des étapes, de l’indice et de la correction. Cette règle s’applique automatiquement à tout TP utilisant `referencedFiles` sans copie locale.
