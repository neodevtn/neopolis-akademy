...
describe('Associate Foundations course 6 contract', () => {
  it('preserves the nine-screen governance sequence and official duration', () => {
    ...
    expect(content('chapter_08')).toContain('Confiance dans les Skills et risque des fonctionnalités');
  ...
  it('restores French governance content, complete cards and Anthropic product names', () => {
    ...
    expect(content('chapter_02')).toContain('#### Source');
    expect(content('chapter_02')).toContain('#### Portée');
    expect(content('chapter_02')).toContain('#### Pertinence');
    expect(content('chapter_02')).not.toContain('### Vérification de la confiance\n\nPortée');
