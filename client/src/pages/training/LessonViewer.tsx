...
import { CloudExerciseBlock, shouldRecordClientCompetency } from "@/components/CloudExerciseBlock";
...
import { GuidedActionBlock } from "@/components/blocks/GuidedActionBlock";
...
import { ComparisonPanelBlock, KnowledgeCheckBlock, LearningObjectivesBlock, LearningProgressBlock, LearningSectionBlock, LearningToolsBlock, LessonSummaryBlock, SequenceVisualBlock } from "@/components/blocks/GenericLearningBlocks";
...
export default function LessonViewer({
  ...
  const submitN8nFoundationsWorkflow = trpc.training.submitN8nFoundationsWorkflow.useMutation();
  ...
  const submitDeveloperFoundationsCheckpoint = trpc.training.submitDeveloperFoundationsCheckpoint.useMutation();
  ...
  const developerFoundationsCheckpointStatus = trpc.training.getDeveloperFoundationsCheckpointStatus.useQuery(
    { courseId: courseId as "claude_certified_developer_foundations__01" | "claude_certified_developer_foundations__02" | "claude_certified_developer_foundations__03" | "claude_certified_developer_foundations__04" | "claude_certified_developer_foundations__05" },
    { enabled: Boolean(user) && /^claude_certified_developer_foundations__0[1-5]$/.test(courseId), retry: false, refetchOnWindowFocus: false },
  );
  ...
  const [completedGuidedActions, setCompletedGuidedActions] = useState<Set<string>>(new Set());
  const [guidedActionResponses, setGuidedActionResponses] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    const completed = guidedActionStatus.data?.completedActionIds;
    if (!completed) return;
    setCompletedGuidedActions((previous) => new Set(Array.from(previous).concat(completed)));
    setGuidedActionResponses((previous) => ({ ...previous, ...Object.fromEntries((guidedActionStatus.data?.completedActions || []).map((action) => [action.id, action.response])) }));
  }, [guidedActionStatus.data]);
  ...
  useEffect(() => {
    const completed = n8nFoundationsActivityStatus.data?.completedPracticalIds;
    if (!completed) return;
    setCompletedCloudExercises((previous) => new Set(Array.from(previous).concat(completed)));
  ...
  useEffect(() => {
    const completed = developerFoundationsCheckpointStatus.data?.passedExerciseIds;
    if (!completed) return;
    setCompletedExercises((previous) => new Set(Array.from(previous).concat(completed)));
  }, [developerFoundationsCheckpointStatus.data]);
  ...
  const isDeveloperFoundationsCourse = /^claude_certified_developer_foundations__0[1-5]$/.test(courseId);
  ...
  [courseId, trpcUtils]);
  const loadDeveloperFoundationsCheckpointCorrection = useCallback((exerciseId: string) =>
    trpcUtils.training.getDeveloperFoundationsCheckpointCorrection.fetch({ courseId: courseId as any, exerciseId }),
  [courseId, trpcUtils]);
  ...
  }, [currentChapter, isLastChapter, completedVideos, flipCardsCompleted, matchingCompleted, completedExercises, completedCloudExercises, completedCourseFinalQuizzes, completedReflections, completedGuidedActions, completedNovasavoInteractions, isReviewMode, chapters]);
  ...
  const renderBlock = (block: any, blockIdx: number) => {
    switch (block.type) {
      ...
      case "learning_objectives":
        return <LearningObjectivesBlock key={blockIdx} block={block} lang={lang} />;
      case "guided_action":
        return <GuidedActionBlock
          key={blockIdx}
          block={block}
          lang={lang}
          completed={completedGuidedActions.has(block.id || `guided_action_${blockIdx}`)}
          savedResponse={guidedActionResponses[block.id || `guided_action_${blockIdx}`] || ""}
          isSubmitting={completeGuidedAction.isPending}
          onComplete={async (actionId, response) => {
            await completeGuidedAction.mutateAsync({ courseId, actionId, lessonIndex, chapterIndex: currentChapter, response });
            setCompletedGuidedActions((previous) => new Set(Array.from(previous).concat(actionId)));
            setGuidedActionResponses((previous) => ({ ...previous, [actionId]: response }));
            trackEventOnce("exercise_complete", `guided-action:${courseId}:${lessonIndex}:${currentChapter}:${actionId}`, { ...analyticsParams, content_id: actionId, status: "completed" });
          }}
        />;
      case "lesson_summary":
        return <LessonSummaryBlock key={blockIdx} block={block} lang={lang} />;
        ...
        return (
          <div key={blockIdx} className="my-4">
            <ExerciseRenderer
              ...
              index={0}
              lang={lang as "en" | "fr"}
              hasServerSubmission={Boolean(exercise.serverCorrectionRequired && (isDeveloperFoundationsCourse ? developerFoundationsCheckpointStatus.data?.submittedExerciseIds.includes(exerciseId) : architectFoundationsCheckpointStatus.data?.submittedExerciseIds.includes(exerciseId)))}
              loadServerSubmissionResult={exercise.serverCorrectionRequired ? (isDeveloperFoundationsCourse ? loadDeveloperFoundationsCheckpointCorrection : loadArchitectFoundationsCheckpointCorrection) : undefined}
              onComplete={async (id, answer, selectedOptionIds) => {
                if (exercise.serverCorrectionRequired === true) {
                  const result = isDeveloperFoundationsCourse
                    ? await submitDeveloperFoundationsCheckpoint.mutateAsync({ courseId: courseId as any, exerciseId: id, answer, selectedOptionIds })
                    : await submitArchitectFoundationsCheckpoint.mutateAsync({ courseId: courseId as any, exerciseId: id, answer, selectedOptionIds });
                  if (isDeveloperFoundationsCourse) await developerFoundationsCheckpointStatus.refetch();
      ...
      case "cloud_exercise": {
        return <CloudExerciseBlock key={blockIdx} block={block} lang={lang} t={t} blockIdx={blockIdx} evaluationContext={{ certificationId: certId, courseId, lessonIndex, chapterIndex: currentChapter }} onEvaluate={(input) => block.serverGradedAssessment === "claude_science_v2_lab"
          ...
          : block.serverGradedAssessment === "n8n_foundations_workflow_json"
            ? submitN8nFoundationsWorkflow.mutateAsync({ courseId: "initiation_automatisation_workflows_n8n__01", blockId: String(input.blockId) as "ch01_ex02_tp" | "ch01_ex04_tp" | "ch01_ex05_tp" | "ch01_ex08_tp" | "ch02_ex08_tp" | "ch02_ex10_tp" | "ch03_ex04_tp" | "ch03_ex06_tp" | "ch03_ex07_tp" | "ch03_ex11_tp", lessonIndex, chapterIndex: currentChapter, workflowJson: String(input.workflowJson || "") })
          ...
          if (outcome && shouldRecordClientCompetency(block, outcome)) recordCompetencyOutcome.mutate({
        ...
        <>
                ...
                <div className="flex items-center gap-2.5 mb-4 flex-wrap">
                  ...
                  {displayedDurationMinutes > 0 && (
                    <span className="reading-time-badge">
                      ...
                      {isClaudeScienceCourse
                        ? t({ en: `${displayedDurationMinutes} min for this lesson`, fr: `${displayedDurationMinutes} min pour cette leçon` })
          ...
          <div className="mt-8 w-full min-w-0 max-w-full border-t border-[#e8e5e0] pt-5 dark:border-slate-700">
            ...
            {(() => {
              ...
              const chapterGuidedActionIds = (chapter?.blocks || []).filter((b: any) => b.type === 'guided_action').map((b: any, i: number) => b.id || `guided_action_${i}`);
              const isGatedByGuidedAction = chapterGuidedActionIds.length > 0 && !chapterGuidedActionIds.every((id: string) => completedGuidedActions.has(id)) && !isReviewMode;
              ...
              const passageConditions = [
                ...
                isGatedByGuidedAction && t({ en: "Confirm the guided action after completing it.", fr: "Confirmez l’action guidée après l’avoir réalisée." }),
              ...
              // Claude Science V2 owns its assessment blueprint: source checkpoints,
              // practical work and two final quizzes. Never inject the generic chapter
              // quiz between its canonical learner screens.
              const usesSourceOwnedAssessmentBlueprint = courseId.startsWith("claude_science_");
              const needsQuiz = isTeachingChapter && !isReviewMode && !chapterQuizPassed.has(currentChapter)
                && courseId !== "automatisation_comptable_ia__01" && !usesSourceOwnedAssessmentBlueprint;

              const isGated = isGatedByExercises || isGatedByVideo || isGatedByFlipCards || isGatedByMatching || isGatedBySingleChoice || isGatedByResourceReview || isGatedByCloudExercise || isGatedByCourseFinalQuiz || isGatedByReflection || isGatedByGuidedAction || isGatedByNovasavo;
              return (
