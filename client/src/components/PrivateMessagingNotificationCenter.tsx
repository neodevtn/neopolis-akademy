    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-w-lg">
        ...
        <div className="max-h-56 space-y-2 overflow-y-auto pr-1">
          {notifications.map((item) => <button key={item.id} type="button" data-conversation-read-state="unread" className="flex w-full items-center justify-between gap-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-left shadow-sm hover:bg-amber-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" onClick={() => { onOpenConversation?.(item.id); setOpen(false); }}><span className="min-w-0"><span className="block truncate text-sm font-bold text-slate-950">{item.subject}</span><span className="mt-0.5 block text-xs text-slate-600">{isAdmin ? item.learner?.name || item.learner?.email || t({ fr: "Apprenant", en: "Learner", ar: "متعلم" }) : t({ fr: "Équipe Neopolis", en: "Neopolis team", ar: "فريق نيوبوليس" })}</span><span className="mt-1 inline-flex items-center gap-1 text-[10px] font-semibold text-amber-900"><span className="h-1.5 w-1.5 rounded-full bg-amber-600" aria-hidden="true" />{t({ fr: "Messages non lus", en: "Unread messages", ar: "رسائل غير مقروءة" })}</span></span><Badge className="bg-amber-600 text-white hover:bg-amber-600" aria-label={t({ fr: `${item.unreadCount} messages non lus`, en: `${item.unreadCount} unread messages`, ar: `${item.unreadCount} رسائل غير مقروءة` })}>{item.unreadCount}</Badge></button>)}
