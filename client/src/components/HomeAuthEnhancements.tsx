...
export default function HomeAuthEnhancements({ slot, fallback = null, onNavigate }: HomeAuthEnhancementProps) {
  ...
  if (slot === "header-primary") {
    ...
    return (
      <Link href="/training" className="flex items-center gap-1.5 text-xs md:text-sm font-semibold px-4 md:px-5 py-2 md:py-2.5 rounded-lg bg-slate-900 text-white hover:bg-slate-800 transition-all duration-200 hover:shadow-md active:scale-[0.97]">
        <LayoutDashboard size={14} /> {t({ fr: "Mon espace", en: "My learning", ar: "مساحتي" })}
      </Link>
    );
  }
