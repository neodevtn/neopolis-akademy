...
export function CourseFinalQuizBlock({ block, courseId, lang, onComplete }: { block: any; courseId: string; lang: string; onComplete: (id: string) => void }) {
  ...
  const canRetry = Boolean(submission && !submission.passed && quiz && (quiz.remainingAttempts === null || quiz.remainingAttempts > 0));
  ...
  return <section className="my-5 overflow-hidden rounded-2xl border border-indigo-200 bg-card shadow-sm">
    <header className="border-b border-indigo-100 bg-indigo-50 px-5 py-4"><div className="flex items-start gap-3"><CircleHelp className="mt-0.5 h-5 w-5 shrink-0 text-indigo-700" /><div><p className="text-xs font-bold uppercase tracking-wider text-indigo-700">{lang === "fr" ? "Évaluation finale" : "Final assessment"} · {lang === "fr" ? "points de compétences" : "competency points"}</p><h3 className="mt-1 text-lg font-bold text-foreground">{title}</h3><p className="mt-1 text-sm text-muted-foreground">{questions.length} {lang === "fr" ? "questions · seuil" : "questions · threshold"} {quiz.passingScore}% · {quiz.remainingAttempts === null ? (lang === "fr" ? "tentatives sans limite" : "unlimited attempts") : `${lang === "fr" ? "tentatives restantes" : "remaining attempts"} : ${quiz.remainingAttempts}`}</p></div></div></header>
