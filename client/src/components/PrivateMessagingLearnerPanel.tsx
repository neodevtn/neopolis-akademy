  ...
  return <section className="space-y-5">
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(280px,0.85fr)_minmax(0,1.4fr)]">
      ...
      <section className="flex min-h-[540px] flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
        {selectedId === null ? <div className="m-auto max-w-sm p-8 text-center"><MessageCircle className="mx-auto h-8 w-8 text-muted-foreground/40" /><h3 className="mt-3 font-semibold text-foreground">Sélectionnez une conversation</h3><p className="mt-1 text-sm text-muted-foreground">L’historique et les actions de réponse seront affichés ici.</p></div> : <>
          <header className="border-b border-border px-4 py-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-base font-semibold text-foreground">{selected?.subject || "Chargement…"}</h3><p className="mt-1 text-xs text-muted-foreground">Équipe Neopolis{selected ? ` · ${privateConversationDisplaySource(selected.source)}` : ""}</p></div>{selected ? <Button variant="outline" size="sm" disabled={statusMutation.isPending} onClick={() => statusMutation.mutate({ conversationId: selectedId, status: selected.status === "open" ? "closed" : "open" })}>{selected.status === "open" ? "Clore" : "Rouvrir"}</Button> : null}</div></header>
          <ScrollArea ref={scrollAreaRef} className="min-h-0 flex-1 px-4 py-4"><div className="space-y-3">
            {detailQuery.isLoading ? <p className="text-sm text-muted-foreground">Chargement des messages…</p> : null}
            {detailQuery.isError ? <p className="text-sm text-destructive">Cette conversation est indisponible.</p> : null}
            {detailQuery.data?.messages.map((message: PrivateMessageView) => <PrivateMessageBubble key={message.id} message={message} mine={message.authorRole === "learner"} author={message.authorRole === "learner" ? "Vous" : "Équipe Neopolis"} dateLabel={formatDate} />)}
            <div ref={bottomRef} aria-hidden="true" />
          </div></ScrollArea>
          <footer className="border-t border-border p-3">{selected?.status === "open" ? <><Label className="sr-only" htmlFor="learner-private-message-reply">Votre réponse</Label><Textarea id="learner-private-message-reply" value={reply} onChange={(event) => setReply(event.target.value)} className="min-h-20 resize-none" maxLength={5000} placeholder="Écrivez votre message à l’équipe Neopolis…" /><PrivateMessageAttachmentPicker attachments={replyAttachments} onChange={setReplyAttachments} disabled={sendMutation.isPending} /><div className="mt-2 flex justify-end"><Button size="sm" className="gap-1.5" disabled={(!reply.trim() && !replyAttachments.length) || sendMutation.isPending} onClick={sendReply}>{sendMutation.isPending ? "Envoi…" : "Envoyer"}<Send className="h-3.5 w-3.5" /></Button></div></> : <p className="text-sm text-muted-foreground">Cette conversation est fermée. Son historique est conservé ; vous pouvez la rouvrir si nécessaire.</p>}</footer>
        </>}
      </section>
    </div>
    <Dialog open={composeOpen} onOpenChange={(open) => open ? setComposeOpen(true) : resetComposer()}><DialogContent><DialogHeader><DialogTitle>{composeMode === "report" ? "Signaler un problème à Neopolis" : "Nouvelle conversation avec Neopolis"}</DialogTitle></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label htmlFor="learner-private-subject">Sujet</Label><Input id="learner-private-subject" value={subject} onChange={(event) => setSubject(event.target.value)} maxLength={220} placeholder="Ex. Question sur mon parcours" /></div><div className="space-y-2"><Label htmlFor="learner-private-body">Message</Label><Textarea id="learner-private-body" value={body} onChange={(event) => setBody(event.target.value)} maxLength={5000} className="min-h-32" placeholder={composeMode === "report" ? "Décrivez le problème rencontré et les étapes concernées…" : "Décrivez votre demande à l’équipe Neopolis…"} /><PrivateMessageAttachmentPicker attachments={composeAttachments} onChange={setComposeAttachments} disabled={createMutation.isPending} /></div></div><DialogFooter><Button variant="outline" onClick={resetComposer}>Annuler</Button><Button disabled={subject.trim().length < 3 || (!body.trim() && !composeAttachments.length) || createMutation.isPending} onClick={() => createMutation.mutate({ subject, body, attachments: composeAttachments.map(({ filename, mimeType, base64 }) => ({ filename, mimeType, base64 })), source: composeMode === "report" ? "problem_report" : "learner" })}>{createMutation.isPending ? "Création…" : "Ouvrir la conversation"}</Button></DialogFooter></DialogContent></Dialog>
  </section>;
}
