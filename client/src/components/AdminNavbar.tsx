...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Messagerie", href: "/admin?tab=messages", icon: MessageSquare, page: "candidatures", description: "Échanger avec les apprenants" },
  ...
  return <>
    <aside className="fixed inset-y-0 left-0 z-50 hidden w-64 flex-col border-r border-slate-200 bg-white lg:flex">
      <div className="flex h-20 items-center gap-3 border-b border-slate-100 px-5">
        <Link href="/admin" className="flex min-w-0 items-center gap-3"><HeaderBrandLogo className="max-w-none" /><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-slate-600">Admin</span></Link>
      </div>
