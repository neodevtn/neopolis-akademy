...
export function CompetencyProfile({ competencies, adminView = false, ranks }: { competencies: Competency[]; adminView?: boolean; ranks?: RankDefinition[] }) {
  ...
  const acquired = competencies.filter((competency) => ["bronze", "silver", "gold"].includes(competency.highestVerifiedRank || ""));
  return <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
    <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
      <div><h3 className="flex items-center gap-2 text-lg font-semibold text-foreground"><Sparkles className="h-5 w-5 text-primary" /> Compétences graduées</h3><p className="mt-1 text-sm text-muted-foreground">{adminView ? "Niveaux calculés à partir des preuves vérifiées de cet apprenant." : "Une compétence exige désormais des preuves variées, plusieurs formations et, aux niveaux avancés, un badge ou une certification."}</p></div>
      <span className="self-start rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary">{acquired.length}/{competencies.length} validée{acquired.length > 1 ? "s" : ""}</span>
    </div>
