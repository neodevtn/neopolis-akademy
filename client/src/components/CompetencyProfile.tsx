...
import { Award, BadgeCheck, ChevronDown, ChevronUp, Sparkles } from "lucide-react";
import { COMPETENCY_PATHS, getCompetencyRank, getNextCompetencyRank } from "@shared/competencyProgression";
  ...
  return <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
    ...
    <div className="mt-5 grid gap-3 md:grid-cols-2">
      {competencies.map((competency) => {
        ...
        const rank = getCompetencyRank(competency.level);
        const nextRank = getNextCompetencyRank(competency.level);
        ...
        return <article key={competency.id} className="rounded-xl border border-border bg-background/40 p-4">
          <button className="w-full text-left" onClick={() => setExpanded(open ? null : competency.id)} aria-expanded={open}>
            <div className="flex items-start gap-3"><div className={`mt-1 h-3 w-3 rounded-full ${colors[competency.color] || "bg-primary"}`} /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><p className="text-sm font-semibold text-foreground">{textOf(competency.title)}</p><p className="mt-0.5 text-xs text-muted-foreground">{competency.category}</p></div><div className="text-right"><span className="text-lg font-bold text-foreground">{competency.level}<small className="ml-0.5 text-xs font-medium text-muted-foreground">/100</small></span><span className={`mt-1 flex items-center justify-end gap-1 text-[11px] font-semibold ${rank.id === "gold" ? "text-amber-600" : rank.id === "silver" ? "text-slate-600" : rank.id === "bronze" ? "text-orange-700" : "text-muted-foreground"}`}><Award className="h-3 w-3" /> {rank.label}</span></div></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-secondary"><div className={`h-full rounded-full ${colors[competency.color] || "bg-primary"}`} style={{ width: `${competency.level}%` }} /></div><p className="mt-1.5 text-xs text-muted-foreground">{competency.rawPoints.toFixed(1)} point{competency.rawPoints !== 1 ? "s" : ""} · {competency.contributionCount} contribution{competency.contributionCount > 1 ? "s" : ""}{nextRank ? ` · ${Math.max(0, nextRank.min - competency.level).toFixed(1)} pour ${nextRank.label}` : " · Niveau maximal atteint"}</p></div>{open ? <ChevronUp className="h-4 w-4 text-muted-foreground" /> : <ChevronDown className="h-4 w-4 text-muted-foreground" />}</div>
          </button>
