...
export function ChoiceQuestionEditor({ block, multiple, onSave, onClose }: EditorProps & { multiple: boolean }) {
  ...
  return <EditorShell title={multiple ? "QCM à choix multiples" : "QCM à choix unique"} onClose={onClose} onSave={() => onSave(data)}>
    ...
    <div className="space-y-1"><Label>Titre facultatif</Label><Input value={localized(data.title, lang)} onChange={(event) => setData((prev: any) => ({ ...prev, title: setLocalized(prev.title, lang, event.target.value) }))} placeholder="Titre affiché au-dessus de la question" /></div>
    ...
    <div className="space-y-1"><Label>Image de contexte facultative</Label><Input value={multiple ? (data.visualAssetUrl || "") : (typeof data.image === "object" ? (data.image?.src || "") : (data.image || ""))} onChange={(event) => setData((prev: any) => multiple ? ({ ...prev, visualAssetUrl: event.target.value }) : ({ ...prev, image: event.target.value }))} placeholder="/api/assets/..." /><p className="text-xs text-muted-foreground">Utilisez une URL issue de la médiathèque Neopolis.</p></div>
    ...
    <div className="space-y-1"><Label>Indice facultatif</Label><Textarea rows={2} value={localized(data.hint, lang)} onChange={(event) => setData((prev: any) => ({ ...prev, hint: setLocalized(prev.hint, lang, event.target.value) }))} placeholder="Aide à afficher sans donner la réponse" /></div>
    {!multiple && <label className="flex items-center gap-2 rounded-lg border p-3 text-sm"><input type="checkbox" checked={data.serverValidated === true} onChange={(event) => setData((prev: any) => ({ ...prev, serverValidated: event.target.checked }))} /> Valider la réponse côté serveur</label>}
...
export function BucketSortBlockEditor({ block, onSave, onClose }: EditorProps) {
  ...
  return <EditorShell title="Tri par glisser-déposer" onClose={onClose} onSave={() => onSave(data)}>
    ...
    <div className="grid gap-3 md:grid-cols-2"><div className="space-y-1"><Label>Indice facultatif</Label><Textarea rows={2} value={localized(data.hint, lang)} onChange={(event) => setData((prev: any) => ({ ...prev, hint: setLocalized(prev.hint, lang, event.target.value) }))} /></div><div className="space-y-1"><Label>Message de réussite</Label><Textarea rows={2} value={localized(data.successMessage, lang)} onChange={(event) => setData((prev: any) => ({ ...prev, successMessage: setLocalized(prev.successMessage, lang, event.target.value) }))} /></div></div>
    <div className="space-y-1"><Label>Feedback après activité</Label><Textarea rows={2} value={localized(data.feedback, lang)} onChange={(event) => setData((prev: any) => ({ ...prev, feedback: setLocalized(prev.feedback, lang, event.target.value) }))} /></div>
  </EditorShell>;
}
...
export function FillBlankBlockEditor({ block, onSave, onClose }: EditorProps) {
  ...
  return <EditorShell title="Texte à trous" onClose={onClose} onSave={() => onSave(data)}>
    ...
    <div className="space-y-1"><Label>Feedback après validation</Label><Textarea rows={2} value={localized(data.feedback, lang)} onChange={(event) => setData((prev: any) => ({ ...prev, feedback: setLocalized(prev.feedback, lang, event.target.value) }))} /></div>
  </EditorShell>;
}
