...
import { ChevronDown, ChevronUp, Dice5, Edit3, GripVertical, Plus, Save, Shuffle, Trash2 } from "lucide-react";
...
export function QuestionBankPanel({ rawBank, onSave, onAddQuestion, onEditQuestion, onRemoveQuestion, onMoveQuestion }: QuestionBankPanelProps) {
  ...
  return (
    <div className="rounded-lg border border-amber-200 bg-amber-50/60 p-3">
      ...
      <div className="mt-3 rounded-md border border-amber-200 bg-white/70 p-3 text-xs text-amber-950"><span className="font-semibold">Aperçu avant publication :</span> l’apprenant devra obtenir <strong>{threshold}/{selectedCount}</strong> bonnes réponses ({Math.ceil((threshold / selectedCount) * 100)} %) {settings.mode === "random" ? `sur ${selectedCount} question${selectedCount > 1 ? "s" : ""} tirée${selectedCount > 1 ? "s" : ""} parmi ${bank.questions.length}` : "sur l’ensemble de la banque"}. {settings.shuffleChoices ? "Les choix seront mélangés." : "Les choix garderont leur ordre."}</div>
      ...
      {bank.questions.length > 0 && <div className="mt-4 space-y-2 border-t border-amber-200 pt-3"><p className="text-xs text-muted-foreground">Glissez la poignée pour réorganiser les questions.</p>{bank.questions.map((question: any, index: number) => <div key={question.id || index} draggable onDragStart={() => setDraggedIndex(index)} onDragEnd={() => setDraggedIndex(null)} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedIndex !== null && draggedIndex !== index) onMoveQuestion?.(draggedIndex, index); setDraggedIndex(null); }} className={`flex items-center gap-2 rounded-md border bg-white/80 p-2 ${draggedIndex === index ? "border-amber-400 opacity-50" : "border-amber-100"}`}><span className="cursor-grab text-amber-700 active:cursor-grabbing" aria-label="Glisser pour réorganiser"><GripVertical className="h-4 w-4" /></span><span className="w-6 text-center text-xs font-bold text-amber-700">{index + 1}</span><p className="min-w-0 flex-1 truncate text-xs font-medium">{typeof question.question === "string" ? question.question : question.question?.fr || question.question?.en || "Question sans énoncé"}</p><Button type="button" size="icon" variant="ghost" className="h-7 w-7" disabled={index === 0} onClick={() => onMoveQuestion?.(index, index - 1)} aria-label="Monter la question"><ChevronUp className="h-3.5 w-3.5" /></Button><Button type="button" size="icon" variant="ghost" className="h-7 w-7" disabled={index === bank.questions.length - 1} onClick={() => onMoveQuestion?.(index, index + 1)} aria-label="Descendre la question"><ChevronDown className="h-3.5 w-3.5" /></Button><Button type="button" size="sm" variant="ghost" className="h-7 gap-1 px-2" onClick={() => onEditQuestion?.(question, index)}><Edit3 className="h-3.5 w-3.5" /> Modifier</Button><Button type="button" size="icon" variant="ghost" className="h-7 w-7 text-red-600 hover:text-red-700" onClick={() => onRemoveQuestion?.(index)} aria-label="Supprimer la question"><Trash2 className="h-3.5 w-3.5" /></Button></div>)}</div>}
    </div>
  );
}
