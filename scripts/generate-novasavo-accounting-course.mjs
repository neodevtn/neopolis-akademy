  ...
  return {
    ...
    chapters: [
      { id: `${prefix}_screen_01`, title: i18n("Objectifs de l’unité"), type: "teaching", requiredBeforeAdvance: false, blocks: [
        { type: "learning_section", sectionKind: "hero", styleVariant: "gradient", styleTone: "brand", id: `${prefix}_hero`, title: i18n(unit.title), body: i18n(`Comprendre les principes de ${topic} pour automatiser sans perdre le contrôle.`), unitNumber: position + 1, duration: "≈ 20 min" },
        { type: "learning_section", sectionKind: "objectives", styleVariant: "soft", id: `${prefix}_objectives`, title: i18n("Ce que cette unité apporte"), items: [i18n(`Identifier les règles et contrôles liés à ${topic}.`), i18n("Distinguer automatisation, assistance et décision humaine."), i18n("Préparer une pratique responsable et traçable.")] },
      ] },
      { id: `${prefix}_screen_02`, title: i18n("Vérification rapide"), type: "teaching", requiredBeforeAdvance: true, blocks: [
        { type: "knowledge_check", mode: "myth_reality", styleVariant: "gradient", id: `${prefix}_myth`, prompt: i18n(`Mythe ou réalité : pour ${topic}, un outil IA fiable dispense de contrôle humain.`), correctAnswer: "mythe", explanation: i18n(keyPoint), competencyPoints: 1, required: true },
      ] },
      { id: `${prefix}_screen_03`, title: i18n("Processus à structurer"), type: "teaching", requiredBeforeAdvance: false, blocks: [
        { type: "sequence_visual", styleLayout: "timeline", id: `${prefix}_timeline`, title: i18n(`Étapes clés : ${topic}`), items: [i18n("Collecter et vérifier les données source"), i18n("Appliquer des règles documentées"), i18n("Traiter les exceptions"), i18n("Valider et journaliser la décision")] },
        { type: "sequence_visual", styleLayout: "flow", id: `${prefix}_flow`, title: i18n("Flux de contrôle"), nodes: [i18n("Données"), i18n("Règles"), i18n("Revue humaine"), i18n("Journal d’audit")] },
      ] },
      { id: `${prefix}_screen_04`, title: i18n("Erreurs à éviter"), type: "teaching", requiredBeforeAdvance: false, blocks: [
        { type: "comparison_panel", styleVariant: "outlined", id: `${prefix}_mistakes`, title: i18n("Pratiques à corriger"), columns: [{ title: i18n("Erreur fréquente"), items: [i18n("Faire confiance à une suggestion sans vérifier la pièce source."), i18n("Utiliser une règle non documentée.")] }, { title: i18n("Correction"), items: [i18n("Prévoir une revue, une exception et une trace de validation."), i18n("Versionner la règle, son responsable et son périmètre.")] }] },
        { type: "learning_tools", toolMode: "assistant", id: `${prefix}_assistant`, title: i18n("Explorer avec l’assistant"), prompt: i18n(`Explique-moi comment préparer un contrôle humain pour ${topic}, sans utiliser de données sensibles.`) },
      ] },
  ...
  return {
    ...
    chapters: [
      ...
      { id: `${prefix}_screen_04`, title: i18n("Partie double et équation comptable"), type: "teaching", requiredBeforeAdvance: true, blocks: [
        ...
        { type: "knowledge_check", mode: "multiple_choice", id: `${prefix}_equation_check`, prompt: i18n("L’équation comptable fondamentale est Actif = Passif + Capitaux propres."), options: [{ id: "vrai", text: i18n("Vrai") }, { id: "faux", text: i18n("Faux") }], correctAnswer: "vrai", explanation: i18n("Correct : cette équation exprime l’équilibre permanent de la partie double."), competencyPoints: 1, required: true },
      ] },
      ...
      { id: `${prefix}_screen_06`, title: i18n("Cycle comptable en un coup d’œil"), type: "teaching", requiredBeforeAdvance: false, blocks: [
        { type: "sequence_visual", styleLayout: "flow", id: `${prefix}_flow`, title: i18n("Le cycle comptable en un coup d’œil"), nodes: steps.map(([title]) => i18n(title)) },
      ] },
      { id: `${prefix}_screen_07`, title: i18n("Les étapes 1 à 4"), type: "teaching", requiredBeforeAdvance: false, blocks: [
        { type: "sequence_visual", styleLayout: "timeline", id: `${prefix}_timeline_1`, title: i18n("Les 8 étapes du cycle comptable manuel · 1 à 4"), items: steps.slice(0, 4).map(([title, description]) => ({ title: i18n(title), description: i18n(description) })) },
      ] },
      { id: `${prefix}_screen_08`, title: i18n("Les étapes 5 à 8"), type: "teaching", requiredBeforeAdvance: false, blocks: [
        { type: "sequence_visual", styleLayout: "timeline", id: `${prefix}_timeline_2`, title: i18n("Les 8 étapes du cycle comptable manuel · 5 à 8"), items: steps.slice(4).map(([title, description]) => ({ title: i18n(title), description: i18n(description) })) },
      ] },
      { id: `${prefix}_screen_09`, title: i18n("Première étape du cycle"), type: "checkpoint", requiredBeforeAdvance: true, blocks: [
        { type: "knowledge_check", mode: "multiple_choice", id: `${prefix}_first_step`, prompt: i18n("Quelle est la première étape du cycle comptable manuel ?"), options: ["Identifier et analyser les transactions", "Enregistrer les transactions dans le journal", "Reporter au grand livre", "Établir la balance de vérification"].map((text, index) => ({ id: ["a", "b", "c", "d"][index], text: i18n(text) })), correctAnswer: "a", explanation: i18n("La première étape consiste à identifier et analyser les transactions à partir des pièces justificatives."), competencyPoints: 2, required: true },
      ] },
      { id: `${prefix}_screen_10`, title: i18n("Erreurs fréquentes"), type: "teaching", requiredBeforeAdvance: false, blocks: [
        { type: "comparison_panel", id: `${prefix}_mistakes`, title: i18n("Erreurs fréquentes"), columns: [{ title: i18n("Erreur"), items: [i18n("Oublier de reporter une écriture du journal vers le grand livre.")] }, { title: i18n("Correction"), items: [i18n("Reportez chaque écriture du journal vers le grand livre afin de conserver des soldes de comptes fiables.")] }] },
      ] },
      ...
      { id: `${prefix}_screen_12`, title: i18n("Assistant comptable"), type: "teaching", requiredBeforeAdvance: false, blocks: [
        { type: "learning_tools", toolMode: "assistant", id: `${prefix}_assistant`, title: i18n("Demande à ton assistant"), prompt: i18n("Comment puis-je automatiser la vérification de ma balance de vérification manuelle dans ma petite entreprise ?"), suggestedQuestions: [i18n("Donne-moi un exemple de contrôle de balance."), i18n("Quelles pièces dois-je vérifier avant une automatisation ?"), i18n("Comment traiter une écriture inhabituelle ?")] },
      ] },
      ...
      { id: `${prefix}_screen_14`, title: i18n("Pratiques de saisie"), type: "teaching", requiredBeforeAdvance: false, blocks: [
        { type: "comparison_panel", id: `${prefix}_comparison`, title: i18n("Tâches manuelles et opportunités d’automatisation"), columns: [{ title: i18n("Saisie des écritures une par une au fil de la journée"), items: [i18n("Correction immédiate possible avant la suite"), i18n("Risque d’interruption du flux de travail")] }, { title: i18n("Saisie par lots à la fin de la journée"), items: [i18n("Correction plus complexe car les erreurs s’accumulent"), i18n("Préparation plus facile à automatiser avec des contrôles")] }] },
      ] },
...
const finalExam = {
  ...
  chapters: Array.from({ length: 5 }, (_, index) => ({ id: `novasavo_exam_${index + 1}`, title: i18n(`Examen final · Question ${index + 1}`), type: "quiz", requiredBeforeAdvance: true, blocks: [{ type: "knowledge_check", mode: "multiple_choice", id: `novasavo_exam_q${index + 1}`, prompt: i18n("Quel principe reste indispensable dans une automatisation comptable responsable ?"), options: options("La validation humaine, la traçabilité et la gestion des exceptions."), correctAnswer: "b", explanation: i18n("L’automatisation soutient le travail comptable ; elle ne supprime ni la responsabilité ni la supervision."), competencyPoints: 3, required: true }] })),
};
