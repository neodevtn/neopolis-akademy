...
const courseSlug = process.env.PROJECTOR_QA_SLUG || "datacamp_ai_for_human_resources";
const courseId = process.env.PROJECTOR_QA_ID || "ai_for_human_resources__01";
...
try {
  ...
  await page.goto(`${baseUrl}/training/${courseSlug}/${courseId}?lesson=0&chapter=0`, { waitUntil: "domcontentloaded" });
  ...
  await page.screenshot({ path: `docs/block-qa-screenshots/${courseId}-projector-mobile.png`, fullPage: true });
  await context.close();
} finally { await browser.close(); }
fs.writeFileSync(`docs/${courseId}_projector_qa_2026-08-28.json`, `${JSON.stringify(result, null, 2)}\n`);
