...
try {
  ...
  if (!target) {
    ...
    const baseAnswer = target.courseId === "ai_for_consulting__01"
      ...
      : target.courseId === "ai_for_finance__01"
        ? "Je souhaite identifier cinq cas d’usage concrets de l’IA générative en finance, notamment pour l’analyse des écarts, les prévisions, la détection d’anomalies, la préparation de commentaires de gestion et la vérification de cohérence. Présentez chaque cas d’usage avec une brève explication de sa valeur, de ses limites et du contrôle humain nécessaire avant toute décision financière."
