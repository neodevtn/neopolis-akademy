...
const coursePath = "client/public/data/courses/building_ai_agents_with_crewai__01.json";
  ...
  for (const chapter of lesson.chapters) {
    for (const block of chapter.blocks ?? []) {
      for (const slide of block.projectorSlides ?? [block]) {
        for (const key of ["content", "contentLeft", "contentRight", "script"]) {
          if (typeof slide[key] === "string" && /https?:\/\//.test(slide[key])) slide[key] = "";
        }
      }
      for (const key of ["script", "transcript"]) {
        if (typeof block[key] === "string" && /https?:\/\//.test(block[key])) block[key] = "";
      }
    }
  }
}
