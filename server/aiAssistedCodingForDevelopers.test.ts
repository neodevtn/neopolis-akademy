...
describe("cours DataCamp Coder avec l’aide de l’IA pour les développeurs", () => {
  ...
  it("publie les compteurs canoniques et le rattachement catalogue", () => {
    ...
    expect(certification).toMatchObject({ group: "fullstack_ai_engineering", totalLessons: 3, totalActivities: 27, totalVideos: 10, totalExercises: 17, totalDownloads: 3 });
    expect(courseIndex).toMatchObject({ certId: certification?.id, totalActivities: 27, videoCount: 10, exerciseCount: 17 });
  });
