...
describe("cours DataCamp L’IA pour la finance", () => {
  ...
  it("respecte les compteurs et le classement BI", () => {
    ...
    expect(certification).toMatchObject({ group: "bi_data_analytics", totalLessons: 3, totalActivities: 30, totalVideos: 10, totalExercises: 20, totalDownloads: 3 });
    expect(courseIndex).toMatchObject({ certId: certification?.id, totalActivities: 30, videoCount: 10, exerciseCount: 20 });
  });
  ...
  it("préserve Projector, TP autonomes, QCM et tris interactifs", () => {
    ...
    expect(cloudExercises).toHaveLength(11);
