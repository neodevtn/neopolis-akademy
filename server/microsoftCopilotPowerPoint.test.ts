...
describe("cours DataCamp Microsoft Copilot dans PowerPoint", () => {
  ...
  it("respecte les compteurs canoniques et la catégorie de productivité", () => {
    ...
    expect(certification).toMatchObject({ group: "workplace_ai_productivity", totalLessons: 3, totalActivities: 14, totalVideos: 7, totalExercises: 7, totalDownloads: 3 });
    expect(courseIndex).toMatchObject({ certId: certification?.id, totalActivities: 14, videoCount: 7, exerciseCount: 7 });
  });
