...
function layout(body: string, options: Parameters<typeof head>[0]) {
  ...
  return `<!doctype html><html lang="${publicTrainingLocaleMeta[options.locale].languageTag}" dir="${publicTrainingLocaleMeta[options.locale].direction}"><head>${head(options)}${styles()}</head><body>
    <header class="site-header"><div class="shell topbar"><a class="brand" href="/" aria-label="Neopolis Akademy"><img class="brand-logo" src="${LOGO_URL}" alt="Neopolis Akademy" width="180" height="44" /></a><nav class="nav" aria-label="${escapeHtml(chrome.program)}"><a href="/#formule">${escapeHtml(chrome.formula)}</a><a href="/#pourquoi">${escapeHtml(chrome.why)}</a><a href="/#partenaires">${escapeHtml(chrome.partners)}</a><a href="${publicTrainingPath(options.locale)}">${escapeHtml(copy.navTraining)}</a><a href="/ai-news">${escapeHtml(copy.navNews)}</a><a href="/#faq">${escapeHtml(chrome.faq)}</a><a href="/login">${escapeHtml(copy.navSignIn)}</a></nav><div class="header-actions"><nav class="language-nav" aria-label="Language">${languageLinks}</nav><details class="mobile-nav"><summary>Menu</summary><nav class="mobile-nav-panel" aria-label="${escapeHtml(chrome.program)}"><a href="/#formule">${escapeHtml(chrome.formula)}</a><a href="/#pourquoi">${escapeHtml(chrome.why)}</a><a href="/#partenaires">${escapeHtml(chrome.partners)}</a><a href="${publicTrainingPath(options.locale)}">${escapeHtml(copy.navTraining)}</a><a href="/ai-news">${escapeHtml(copy.navNews)}</a><a href="/#faq">${escapeHtml(chrome.faq)}</a><a href="/login">${escapeHtml(copy.navSignIn)}</a><nav class="mobile-language-nav" aria-label="Language">${languageLinks}</nav></nav></details><a class="apply-link" href="/apply">${escapeHtml(chrome.apply)}</a></div></div></header>
