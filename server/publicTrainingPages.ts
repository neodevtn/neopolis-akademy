...
function head({ locale, title, description, canonicalPath, noindex = false, schema, themeSlug }: {
...
function styles() {
  return `<style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 640px) { .shell { width: min(100% - 24px, 1180px); }.topbar { min-height: 64px; }.nav a { padding: 7px 6px; }.nav a:nth-child(2) { display: none; }.language-nav { display: none; }.hero { padding: 48px 0 40px; }.metrics { grid-template-columns: repeat(2, minmax(0, 1fr)); }.themes, .training-grid { grid-template-columns: 1fr; }.metric { padding: 14px; } }
  </style>`;
}
...
export function renderPublicTrainingIndex(locale: PublicTrainingLocale = "fr") {
  ...
  return layout(body, { locale, title: `${copy.indexTitle} | ${SITE_NAME}`, description: copy.menaDescription, canonicalPath: publicTrainingPath(locale), schema: publicIndexSchema(locale) });
}
...
export function registerPublicTrainingPages(app: Express) {
  ...
  const theme = (locale: PublicTrainingLocale) => (req: Request, res: Response) => {
    ...
    return resolved ? sendHtml(res, renderPublicTrainingTheme(resolved, locale)) : sendHtml(res, renderPublicTrainingNotFound(locale), 404);
  };
