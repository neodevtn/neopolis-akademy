...
describe("cours DataCamp Concevoir des systèmes agentiques évolutifs", () => {
  ...
  it("publie les compteurs canoniques et le rattachement catalogue", () => {
    ...
    expect(certification).toMatchObject({ group: "claude_ai_agents", totalLessons: 3, totalActivities: 29, totalVideos: 10, totalExercises: 19, totalDownloads: 3 });
  ...
  it("préserve les médias Projector locaux, l’interactivité et la progression séquentielle", () => {
    ...
    expect(blocks.filter((block: any) => block.type === "multi_choice_exercise" || block.type === "single_choice_exercise")).toHaveLength(13);
    ...
    expect(activities.filter((activity: any) => activity.sourceActivityType === "VisualExercise")).toHaveLength(3);
  ...
  it("n’expose aucune URL DataCamp externe dans les données publiées", () => {
