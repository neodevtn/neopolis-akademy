...
describe("cours DataCamp Introduction au Model Context Protocol", () => {
  ...
  it("respecte les compteurs canoniques et le classement développeur", () => {
    ...
    expect(certification).toMatchObject({ group: "fullstack_ai_engineering", totalLessons: 3, totalActivities: 30, totalVideos: 11, totalExercises: 17, totalDownloads: 3 });
    expect(courseIndex).toMatchObject({ certId: certification?.id, totalActivities: 30, videoCount: 11, exerciseCount: 17 });
  });
